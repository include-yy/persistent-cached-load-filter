...
=load-path-filter-function= variable introduced in Emacs 31. By caching
the directory locations of loaded libraries to a file, it reduces the
number of system calls (stat/access) required when =require= or =load=
is called, particularly beneficial for systems with slow I/O or large
=load-path= lists.
...
#+begin_src elisp
  ...
  ;; (setq persistent-cached-load-filter-assoc-type 'hash+equal)
...
- *Hash Tables (Fastest Lookup)*
  - Symbol: ='hash+equal= (String keys), ='hash+eq= (Interned symbol keys)
